...
def test_valid_transaction(benchmark):
    testMessageValidation = TestMessageValidation()

    @benchmark
    def do_valid_transaction():
        txn_list = testMessageValidation._create_transactions(1)
        txn = txn_list[0]
        valid = verifier.is_valid_transaction(txn)
        testMessageValidation.assertTrue(valid)


def test_valid_batch(benchmark):
    testMessageValidation = TestMessageValidation()

    @benchmark
    def do_valid_batch():
        batch_list = testMessageValidation._create_batches(1, 10)
        batch = batch_list[0]
        valid = verifier.is_valid_batch(batch)
        testMessageValidation.assertTrue(valid)


def test_valid_block(benchmark):
    testMessageValidation = TestMessageValidation()

    @benchmark
    def do_valid_block():
        block_list = testMessageValidation._create_blocks(1, 1)
        block = block_list[0]
