...
class Monitor {
    ...
    start() {
        ...
        if (typeof m === "undefined") {
            return Promise.reject(
                new Error("Failed to find monitor type in config file")
            );
        }
                ...
                monitorTypes.forEach(type => {
                    promises.push(
                        new Promise((resolve, reject) => {
                            let p = null;
                            if (type === "docker") {
                                // monitor for local docker containers
                                p = this._startDockerMonitor(
                                    m.docker,
                                    m.interval
                                );
                            } else if (type === "process") {
                                p = this._startProcessMonitor(
                                    m.process,
                                    m.interval
                                );
                            } else {
    ...
    printMaxStats() {
        try {
            ...
            var t = table.table(defaultTable, {
    ...
    _readDefaultStats(tmp) {
        ...
        this.peers.forEach(peer => {
            ...
            if (peer.isLastTmp) {
                ...
                peer.history["Traffic In"][lastIdx] =
                    net.in[net.in.length - 1] - net.in[0];
                peer.history["Traffic Out"][lastIdx] =
                    net.out[net.out.length - 1] - net.out[0];
            } else {
                ...
                peer.history["Traffic In"].push(
                    net.in[net.in.length - 1] - net.in[0]
                );
                peer.history["Traffic Out"].push(
                    net.out[net.out.length - 1] - net.out[0]
                );
            }
            peer.isLastTmp = tmp;
    ...
    _getMaxHistoryValues(items, idx) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (key.indexOf("Memory") === 0 || key.indexOf("Traffic") === 0) {
                ...
                values.push((stats.max || 0).toFixed(2) + "%");
            } else {
