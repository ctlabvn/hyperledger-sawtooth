...
class Blockchain {
    ...
    invokeSmartContract(context, contractID, contractVer, args, timeout) {
        if (typeof timeout !== "number" || timeout < 0) {
            return this.bcObj.invokeSmartContract(
                context,
                contractID,
                contractVer,
                args,
    ...
    static mergeDefaultTxStats(results) {
        try {
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (
                    !result.hasOwnProperty("succ") ||
                    !result.hasOwnProperty("fail") ||
                    result.succ + result.fail === 0
                ) {
                    skip++;
                } else {
                    break;
            ...
            for (let i = 1; i < results.length; i++) {
                let v = results[i];
                if (
                    !v.hasOwnProperty("succ") ||
                    !v.hasOwnProperty("fail") ||
                    v.succ + v.fail === 0
                ) {
                    continue;
                }
