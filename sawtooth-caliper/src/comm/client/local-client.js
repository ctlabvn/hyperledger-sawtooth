...
function txUpdate() {
    ...
    process.send({
        type: "txUpdated",
        data: { submitted: newNum, committed: newStats }
    });
}
...
function doTest(msg) {
    console.log("doTest() with: ", msg.config.file);
    let cb = require(path.join(__dirname, "../../..", msg.cb));
    // we pass config directly
    ...
    return blockchain
        .getContext(msg.label, msg.clientargs)
        .then(context => {
            if (msg.txDuration) {
        ...
        .then(() => {
            ...
            if (msg.trim) {
                ...
                if (msg.txDuration) {
                    ...
                    trim = Math.floor(
                        msg.trim * (results.length / msg.txDuration)
                    );
                } else {
