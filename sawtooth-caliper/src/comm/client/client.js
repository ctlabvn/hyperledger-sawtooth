...
class Client {
            ...
            switch (this.config.type) {
                ...
                default:
                    return Promise.reject(
                        new Error(
                            "Unknown client type, should be local or zookeeper"
                        )
                    );
            }
        } else {
            return Promise.reject(
                new Error("Failed to find client type in config file")
            );
        }
    ...
    startTest(message, clientArgs, finishCB, finshArgs) {
        ...
        switch (this.type) {
            ...
            default:
                return Promise.reject(
                    new Error("Unknown client type: " + this.type)
                );
        }
        return p
            .then(() => {
    ...
    _initZoo() {
        ...
        if (configZoo.hasOwnProperty("server")) {
            ...
            return Promise.reject(
                new Error(
                    "Failed to find zookeeper server address in config file"
                )
            );
        }
    ...
    _startZooTest(message, clientArgs) {
        ...
        if (message.numb) {
            ...
            return reject(
                new Error("Unconditioned transaction rate driving mode")
            );
        }
        ...
        return this._sendZooMessage(message, clientArgs)
            .then(number => {
                if (number > 0) {
                    ...
                    return Promise.reject(
                        new Error("Failed to start the remote test")
                    );
                }
            })
            .catch(err => {
    ...
    _sendZooMessage(message, clientArgs) {
        ...
        this.zoo.hosts.forEach((host, idx) => {
            ...
            if (Array.isArray(clientArgs)) {
                let msg = message;
                msg["clientargs"] = clientArgs.slice(
                    idx * argsSlice,
                    idx * argsSlice + argsSlice
                );
            ...
            let p = zkUtil
                .createP(
                    ...
                    "Failed to send message (create node) due to"
                )
                .then(path => {
                    succ++;
                    return Promise.resolve();
        ...
        let p = zkUtil.watchMsgQueueP(
            ...
            data => {
                return zooMessageCallback(data, updates, results).catch(err => {
                    console.log(
                        "Exception encountered when watching message from zookeeper, due to:"
                    );
                    console.log(err);
